...
const assertShownLegendShiftEscapeNoOpAcrossEscapeKeysByEditableAndMode = (
  isEditableTarget: boolean,
  mode: LegendNoOpAssertionMode,
) => {
  forEachLegendEscapeKey((key) => {
...
const assertLegendModifierEventGateNoOpAcrossEscapeKeysByVisibilityShiftEditableAndMode = (
  isVisible: boolean,
  shiftKey: boolean,
  isEditableTarget: boolean,
  mode: LegendNoOpAssertionMode,
...
describe('thread shortcut legend lifecycle presentation (main integration)', () => {
  ...
  it('keeps shown Escape/Esc as no-op per-mode parity when event is defaultPrevented or repeat across editable states', () => {
    ;([false, true] as const).forEach((isEditableTarget) => {
      forEachLegendNoOpAssertionMode((mode) => {
        assertShownLegendEventGateNoOpAcrossEditableAndMode(isEditableTarget, mode)
      })
    })
  })
  ...
  it('keeps shown Escape/Esc modifier-key paths as no-op per-mode parity outcomes across editable + shift states', () => {
    ;([
  ...
  it('keeps shown Escape/Esc with shiftKey=true modifier+event-gate paths as no-op per-mode parity outcomes across editable states', () => {
    ;([true, false] as const).forEach((isEditableTarget) => {
      forEachLegendNoOpAssertionMode((mode) => {
        assertLegendModifierEventGateNoOpAcrossEscapeKeysByVisibilityShiftEditableAndMode(
          true,
          true,
          isEditableTarget,
          mode,
