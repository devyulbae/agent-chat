...
const assertHiddenEditableLegendEventGateNoOp = (
  key: 'Escape' | 'Esc',
  mode: LegendEventGateAssertionMode,
) => {
  legendEventGateCases.forEach(({ defaultPrevented, repeat }) => {
    assertHiddenEditableLegendNoOpByMode(key, defaultPrevented, repeat, 'dispatch')

    if (mode === 'dispatch+render') {
      assertHiddenEditableLegendNoOpByMode(key, defaultPrevented, repeat, 'render')
    }
  })
}
...
const assertHiddenLegendModifierNoOpDispatch = (key: 'Escape' | 'Esc') => {
  legendModifierCases.forEach(({ metaKey, ctrlKey, altKey }) => {
    assertHiddenLegendNoOpDispatch(key, { metaKey, ctrlKey, altKey })
  })
}
...
const assertHiddenLegendEventGateNoOp = (
  ...
  assertLegendEventGateNoOpByVisibilityAndMode(key, false, mode)
}
...
const assertShownLegendEventGateNoOpRenderState = (key: 'Escape' | 'Esc') => {
  assertLegendEventGateNoOpByVisibilityAndMode(key, true, 'dispatch+render')
}
...
describe('thread shortcut legend lifecycle presentation (main integration)', () => {
  ...
  it('keeps hidden Escape with modifier keys as no-op dispatch outcomes matching keyboard guard rails', () => {
    assertHiddenLegendModifierNoOpDispatch('Escape')
  })

  it('keeps hidden Esc alias with modifier keys as no-op dispatch outcomes', () => {
    assertHiddenLegendModifierNoOpDispatch('Esc')
  })
  ...
  it('keeps hidden Escape as no-op dispatch/render-state when target is editable and event is defaultPrevented or repeat', () => {
    assertHiddenEditableLegendEventGateNoOp('Escape', 'dispatch+render')
  })
  ...
  it('keeps shown Escape with shiftKey=true as no-op for editable target when event is defaultPrevented or repeat', () => {
    assertShownEditableLegendEventGateNoOpByShift('Escape', true)
  })

  it('keeps shown Esc alias with shiftKey=true as no-op for editable target when event is defaultPrevented or repeat', () => {
    assertShownEditableLegendEventGateNoOpByShift('Esc', true)
  })

  it('keeps shown Escape/Esc editable-target modifier-key paths as no-op dispatch outcomes', () => {
    assertShownEditableLegendModifierNoOpByShift('Escape', false)
    assertShownEditableLegendModifierNoOpByShift('Esc', false)
  })
